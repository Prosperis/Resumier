...
export const LINKEDIN_CONFIG = {
  ...
  redirectUri: import.meta.env.VITE_LINKEDIN_REDIRECT_URI || "http://localhost:5173/auth/linkedin/callback",
 ...
 */
export function getLinkedInAuthUrl(state: string, scope: string[] = ["openid", "profile", "email"]): string {
...
export async function exchangeCodeForToken(code: string): Promise<{
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({ error: "Failed to exchange authorization code for token" }));
...
export async function fetchLinkedInEmail(accessToken: string): Promise<string> {
  const response = await fetch(`${LINKEDIN_CONFIG.apiUrl}/emailAddress?q=members&projection=(elements*(handle~))`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
      "LinkedIn-Version": "202310",
    },
 ...
 */
export async function fetchLinkedInCertifications(accessToken: string): Promise<LinkedInCertification[]> {
 ...
 */
export async function importFromLinkedInOAuth(accessToken: string): Promise<Partial<ResumeContent>> {
  try {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: "Failed to fetch LinkedIn profile" }));
