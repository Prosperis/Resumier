  ...
  return (
    <div className="mx-auto max-w-[21cm] bg-white text-gray-900 shadow-lg p-10 font-serif">
      <div className="border-t-4 border-b-4 py-4 mb-6" style={{ borderColor: primaryColor }}>
        <h1 className="text-center text-3xl font-bold uppercase tracking-widest mb-2">{personalInfo.name || "YOUR NAME"}</h1>
      ...
      {personalInfo.summary && (
        <div className="mb-6">
          <h2 className="text-center text-lg font-bold uppercase tracking-wide mb-3 pb-2 border-b-2" style={{ borderColor: primaryColor, color: primaryColor }}>
        ...
        <div className="mb-6">
          ...
          {experience.map((exp, idx) => (
            <div key={idx} className="mb-4">
              <div className="flex justify-between items-start border-b pb-2 mb-2" style={{ borderColor: `${primaryColor}30` }}>
                <div>
            ...
            <ul className="space-y-1">
              {typeof skills === 'object' && 'technical' in skills && Array.isArray(skills.technical) &&
                skills.technical.map((skill: any, idx: number) => (
                  <li key={idx} className="text-sm text-gray-700">• {skill.name || skill}</li>
                ))}
            </ul>
          </div>
        )}
