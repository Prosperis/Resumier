...
export function ProfessionalServicesTemplate({ resume, config }: ProfessionalServicesTemplateProps) {
    ...
    <div className="mx-auto max-w-[21cm] bg-white text-gray-900 shadow-lg p-8">
      <div className="flex justify-between items-start mb-6 pb-4 border-b-2" style={{ borderColor: primaryColor }}>
        ...
        <div className="text-right">
          {personalInfo.location && <p className="text-sm text-gray-600">{personalInfo.location}</p>}
        </div>
      </div>
      ...
      {personalInfo.summary && (
        <div className="mb-6">
          <h2 className="text-xl font-bold mb-3" style={{ color: primaryColor }}>Professional Profile</h2>
      ...
      {experience && experience.length > 0 && (
        <div className="mb-6">
          ...
          {experience.map((exp, idx) => (
            <div key={idx} className="mb-4 p-3 rounded" style={{ backgroundColor: `${primaryColor}05`, borderLeft: `3px solid ${primaryColor}` }}>
      ...
      <div className="grid grid-cols-2 gap-6">
        {skills && (
          <div>
            <h2 className="text-xl font-bold mb-3" style={{ color: primaryColor }}>Core Competencies</h2>
            <div className="grid grid-cols-2 gap-2">
              {typeof skills === 'object' && 'technical' in skills && Array.isArray(skills.technical) &&
                skills.technical.map((skill: any, idx: number) => (
                  <div key={idx} className="p-2 text-sm text-center rounded" style={{ backgroundColor: `${primaryColor}10` }}>
                    {skill.name || skill}
                  </div>
                ))}
            </div>
