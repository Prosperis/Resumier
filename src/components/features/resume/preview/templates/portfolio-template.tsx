...
export function PortfolioTemplate({ resume, config }: PortfolioTemplateProps) {
  ...
  return (
    <div className="mx-auto max-w-[21cm] bg-white text-gray-900 shadow-lg">
      <div className="p-10" style={{ background: `linear-gradient(135deg, ${primaryColor} 0%, ${primaryColor}cc 100%)`, color: "white" }}>
      ...
      <div className="p-10">
        {personalInfo.summary && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold mb-3" style={{ color: primaryColor }}>About Me</h2>
        ...
        {experience && experience.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold mb-4 flex items-center gap-2" style={{ color: primaryColor }}>
              <Briefcase className="h-6 w-6" />
              Featured Projects
            </h2>
            <div className="grid grid-cols-2 gap-4">
              {experience.map((exp, idx) => (
                <div key={idx} className="p-4 border-2 rounded-lg hover:shadow-md transition-shadow" style={{ borderColor: `${primaryColor}30` }}>
                  <div className="h-24 mb-3 rounded flex items-center justify-center text-white font-bold text-xl" style={{ backgroundColor: primaryColor }}>
                    {exp.position.substring(0, 2).toUpperCase()}
                  </div>
                  <h3 className="font-bold mb-1">{exp.position}</h3>
                  <p className="text-sm text-gray-600 mb-2">{exp.company}</p>
                  {exp.description && <p className="text-xs text-gray-700 line-clamp-2">{exp.description}</p>}
                </div>
              ))}
            </div>
          </div>
                  ...
                  skills.technical.slice(0, 8).map((skill: any, idx: number) => (
                    <div key={idx}>
                      ...
                      <div className="w-full h-2 bg-gray-200 rounded-full">
                        <div className="h-2 rounded-full" style={{ width: `${skill.level || 85}%`, backgroundColor: primaryColor }} />
                      </div>
                    </div>
                  ))}
              </div>
            ...
            {education && education.length > 0 && (
              <div className="mb-6">
                ...
                {education.map((edu, idx) => (
                  <div key={idx} className="mb-3 p-3 rounded" style={{ backgroundColor: `${primaryColor}08` }}>
