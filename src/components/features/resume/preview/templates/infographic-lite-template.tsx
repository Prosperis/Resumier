  ...
  return (
    <div className="mx-auto max-w-[21cm] bg-white text-gray-900 shadow-lg p-8">
      <div className="flex items-center gap-6 mb-8">
        <div className="w-24 h-24 rounded-full flex items-center justify-center text-white text-3xl font-bold" style={{ backgroundColor: primaryColor }}>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="p-4 text-center rounded" style={{ backgroundColor: `${primaryColor}10` }}>
          <div className="text-3xl font-bold" style={{ color: primaryColor }}>{experience?.length || 0}</div>
          <div className="text-sm text-gray-600">Years Experience</div>
        </div>
        <div className="p-4 text-center rounded" style={{ backgroundColor: `${primaryColor}10` }}>
          <div className="text-3xl font-bold" style={{ color: primaryColor }}>{education?.length || 0}</div>
          <div className="text-sm text-gray-600">Degrees</div>
        </div>
        <div className="p-4 text-center rounded" style={{ backgroundColor: `${primaryColor}10` }}>
          <div className="text-3xl font-bold" style={{ color: primaryColor }}>
            {typeof skills === 'object' && 'technical' in skills && Array.isArray(skills.technical) ? skills.technical.length : 0}
          </div>
      ...
      {experience && experience.length > 0 && (
        <div className="mb-6">
          ...
          {experience.map((exp, idx) => (
            <div key={idx} className="mb-4 p-3 rounded border-l-4" style={{ borderColor: primaryColor, backgroundColor: `${primaryColor}05` }}>
            ...
            <div className="flex flex-wrap gap-2">
              {typeof skills === 'object' && 'technical' in skills && Array.isArray(skills.technical) &&
                skills.technical.map((skill: any, idx: number) => (
                  <span key={idx} className="px-3 py-1 rounded text-white text-xs" style={{ backgroundColor: primaryColor }}>
                    {skill.name || skill}
                  </span>
                ))}
            </div>
        ...
        {education && education.length > 0 && (
          <div>
            ...
            {education.map((edu, idx) => (
              <div key={idx} className="mb-2 p-2 rounded" style={{ backgroundColor: `${primaryColor}08` }}>
