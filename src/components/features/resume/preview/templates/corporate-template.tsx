  ...
  return (
    <div className="mx-auto max-w-[21cm] bg-white p-12 text-gray-900 shadow-lg font-serif">
      ...
      {personalInfo.summary && (
        <div className="mb-6">
          <h2 className="text-sm font-bold uppercase tracking-wide mb-2 border-b border-gray-300 pb-1">Professional Summary</h2>
          <p className="text-sm leading-relaxed">{personalInfo.summary}</p>
        </div>
      )}
      {experience && experience.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-bold uppercase tracking-wide mb-2 border-b border-gray-300 pb-1">Professional Experience</h2>
            ...
            <div key={idx} className="mb-5">
              ...
              {exp.highlights && exp.highlights.length > 0 && (
                <ul className="text-sm mt-2 space-y-1">
                  {exp.highlights.map((highlight, i) => <li key={i} className="ml-4">• {highlight}</li>)}
                </ul>
              )}
            </div>
          ))}
      ...
      {education && education.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-bold uppercase tracking-wide mb-2 border-b border-gray-300 pb-1">Education</h2>
      ...
      {skills && (
        <div className="mb-6">
          <h2 className="text-sm font-bold uppercase tracking-wide mb-2 border-b border-gray-300 pb-1">Professional Skills</h2>
          <div className="grid grid-cols-2 gap-2">
            {typeof skills === 'object' && 'technical' in skills && Array.isArray(skills.technical) &&
              skills.technical.map((skill: any, idx: number) => <span key={idx} className="text-sm">• {skill.name || skill}</span>)}
          </div>
        </div>
      )}
      {certifications && certifications.length > 0 && (
        <div>
          <h2 className="text-sm font-bold uppercase tracking-wide mb-2 border-b border-gray-300 pb-1">Professional Certifications</h2>
