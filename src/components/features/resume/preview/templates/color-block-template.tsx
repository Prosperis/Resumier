...
export function ColorBlockTemplate({ resume, config }: ColorBlockTemplateProps) {
    ...
    <div className="mx-auto max-w-[21cm] bg-white text-gray-900 shadow-lg">
      <div className="grid grid-cols-3">
        <div className="col-span-1 p-8 text-white" style={{ backgroundColor: primaryColor }}>
          <h1 className="text-3xl font-bold mb-4">{personalInfo.name || "Your Name"}</h1>
        ...
        <div className="col-span-2 p-8">
          {personalInfo.summary && (
            <div className="mb-6 p-4" style={{ backgroundColor: `${primaryColor}15` }}>
          ...
          {experience && experience.length > 0 && (
            <div className="mb-6">
              <div className="p-2 mb-3 text-white font-bold" style={{ backgroundColor: primaryColor }}>
              ...
              {experience.map((exp, idx) => (
                <div key={idx} className="mb-4">
                  ...
                  <p className="text-sm text-gray-700">{exp.company} | {exp.startDate} - {exp.current ? "Present" : exp.endDate}</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            {skills && (
              <div>
                <div className="p-2 mb-3 text-white font-bold" style={{ backgroundColor: primaryColor }}>SKILLS</div>
                <div className="flex flex-wrap gap-2">
                  {typeof skills === 'object' && 'technical' in skills && Array.isArray(skills.technical) &&
                    skills.technical.slice(0, 8).map((skill: any, idx: number) => (
                      <span key={idx} className="px-2 py-1 text-xs text-white" style={{ backgroundColor: primaryColor }}>
                        {skill.name || skill}
                      </span>
                    ))}
                </div>
