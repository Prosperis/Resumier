      ...
      {experience && experience.length > 0 && (
        <div className="mb-10">
          <div className="flex items-center gap-4 mb-6">
            ...
            <h2 className="text-2xl font-light tracking-wider flex items-center gap-2" style={{ color: primaryColor }}>
          ...
          {experience.map((exp, idx) => (
            <div key={idx} className="mb-8 max-w-3xl mx-auto">
              ...
              {exp.description && <p className="text-gray-700 leading-relaxed mb-2">{exp.description}</p>}
      ...
      <div className="grid grid-cols-2 gap-10">
        {education && education.length > 0 && (
          <div>
            <div className="flex items-center gap-4 mb-4">
              <h2 className="text-xl font-light tracking-wider flex items-center gap-2" style={{ color: primaryColor }}>
        ...
        <div>
          {skills && (
            <div className="mb-6">
              <div className="flex items-center gap-4 mb-4">
                <h2 className="text-xl font-light tracking-wider" style={{ color: primaryColor }}>Skills</h2>
                <div className="flex-1 h-px" style={{ backgroundColor: primaryColor }} />
              </div>
              <div className="space-y-2">
                {typeof skills === 'object' && 'technical' in skills && Array.isArray(skills.technical) &&
                  skills.technical.map((skill: any, idx: number) => (
                    <p key={idx} className="text-sm text-gray-700">{skill.name || skill}</p>
                  ))}
              </div>
            </div>
          )}
