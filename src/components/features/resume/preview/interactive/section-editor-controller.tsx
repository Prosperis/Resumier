...
export function SectionEditorController() {
    ...
    switch (type) {
      ...
      case "experience":
        if (itemId) {
          ...
          return exp ? { type, data: exp, index, total: content.experience.length } : null;
        }
        return null;
      ...
      case "education":
        if (itemId) {
          ...
          return edu ? { type, data: edu, index, total: content.education.length } : null;
        }
        return null;
      ...
      case "certifications":
        if (itemId) {
          ...
          const index = content.certifications.findIndex((c) => c.id === itemId);
          return cert ? { type, data: cert, index, total: content.certifications.length } : null;
        }
        return null;
      ...
      case "links":
        if (itemId) {
          ...
          return link ? { type, data: link, index, total: content.links.length } : null;
        }
        return null;
  ...
  const navigateToItem = useCallback(
    (direction: "prev" | "next") => {
      if (!selectedSection || !sectionData || sectionData.index === undefined) return;
      ...
      const newIndex = direction === "prev" ? sectionData.index - 1 : sectionData.index + 1;
  ...
  const hasNext = isListItem && sectionData.index < (sectionData.total || 0) - 1;
      ...
      case "experience":
        return (
          <ExperienceEditor
            ...
            onChange={(data) => onUpdateExperience(selectedSection.itemId!, data)}
          />
        );
      ...
      case "education":
        return (
          <EducationEditor
            ...
            onChange={(data) => onUpdateEducation(selectedSection.itemId!, data)}
          />
        );

      case "skills":
        return <SkillsEditor data={sectionData.data as Skills} onChange={onUpdateSkills} />;

      case "certifications":
        return (
          <CertificationEditor
            data={sectionData.data as Certification}
            onChange={(data) => onUpdateCertification(selectedSection.itemId!, data)}
          />
        );
